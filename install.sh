...
install_V2bX() {
    ...
    if  [ $# == 0 ] ;then
        ...
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}检测 bX 版本失败，可能是超出 Github API 限制，请稍后再试，或手动指定 bX 版本安装${plain}"
            exit 1
        fi
        echo -e "检测到 bX 最新版本：${last_version}，开始安装"
    ...
    if [[ ! -f /etc/V2bX/config.json ]]; then
        ...
        echo -e ""
        if [[ $? == 0 ]]; then
            echo -e "${green}bX 重启成功${plain}"
        else
            echo -e "${red}bX 可能启动失败，请稍后使用 bX log 查看日志信息${plain}"
        fi
        first_install=false
    fi
    ...
    rm -f install.sh
    echo -e ""
    echo "V2bX 管理脚本使用方法 (兼容使用bX执行，大小写不敏感): "
    echo "------------------------------------------"
    echo "V2bX              - 显示管理菜单 (功能更多)"
    echo "V2bX start        - 启动 bX"
    echo "V2bX stop         - 停止 bX"
    echo "V2bX restart      - 重启 bX"
    echo "V2bX status       - 查看 bX 状态"
    echo "V2bX enable       - 设置 bX 开机自启"
    echo "V2bX disable      - 取消 bX 开机自启"
    echo "V2bX log          - 查看 bX 日志"
    echo "V2bX x25519       - 生成 x25519 密钥"
    echo "V2bX generate     - 生成 bX 配置文件"
    echo "V2bX update       - 更新 bX"
    echo "V2bX update x.x.x - 更新 bX 指定版本"
    echo "V2bX install      - 安装 bX"
    echo "V2bX uninstall    - 卸载 bX"
    echo "V2bX version      - 查看 bX 版本"
    echo "------------------------------------------"
    # 首次安装询问是否生成配置文件
    if [[ $first_install == true ]]; then
        read -rp "检测到你为第一次安装bX,是否自动直接生成配置文件？(y/n): " if_generate
